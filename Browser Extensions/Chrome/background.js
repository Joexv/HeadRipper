...
chrome.runtime.onInstalled.addListener(d=>{ log("onInstalled:", d); ensureHostPermissions(); });
chrome.runtime.onStartup?.addListener(()=>{ log("onStartup"); ensureHostPermissions(); });

try {
  chrome.alarms.create("heartbeat", { periodInMinutes: 1 });
  chrome.alarms.onAlarm.addListener(a => { if (a?.name === "heartbeat") log("heartbeat"); });
  log("alarms heartbeat installed");
} catch (e) { err("heartbeat setup failed:", e); }

chrome.storage.session.get(["bearer"]).then(st => {
...
async function ensureHostPermissions() {
  try {
    const have = await chrome.permissions.contains({ origins: ORIGINS });
...
async function getBearer() {
  ...
  const st = await chrome.storage.session.get(["bearer"]);
  bearer = st.bearer || null;
  return bearer;
}
