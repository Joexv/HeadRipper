...
def fetch_variants(entity_id, headers):
...
def interactive_flow(args):
    ...
    folder = item.get("category") or title
    out_dir = AUDIO_DIR / location / topic_name / folder
    ...
    if args.variant == "manual":
        ...
        download_audio(vid, out_path, headers)
    else:
        for v in variants:
            vid = v.get("id")
            narr = v.get("narrator", "")
...
def direct_flow(args):
    ...
    for it in items:
        ...
        out_dir = AUDIO_DIR / args.location / it["category"]
        out_dir.mkdir(parents=True, exist_ok=True)
        variants = fetch_variants(cid, headers)
        if variants:
            for vid, attrs in variants:
                narr = attrs.get("narrator", "")
                ...
                fname += f"-{vid}.mp3"
                out_path = out_dir / fname
                download_audio(vid, out_path, headers, args.container)
        else:
            out_path = out_dir / f"{title}.{args.container}"
            download_audio(cid, out_path, headers, args.container)
